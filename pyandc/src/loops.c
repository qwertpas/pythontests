#include <stdio.h>
#include <iostream>

#define Q16_2_3 ((uint16_t) 43691) 		// (2/3) * 2^16
#define Q16_SQRT3_2 ((uint16_t) 56756) 	// (sqrt(3)/2) * 2^16
#define Q16_1_2 ((uint16_t) 32768) 		// (1/2) * 2^16

#define SQRT3 		1.73205080757f
#define SQRT3_2 	0.86602540378f
#define SQRT1_3		0.57735026919f
#define TWO_PI_F 	6.28318548203f

static const int16_t sin_lut[] = { //sine wave with 256 values from -32,768 to 32,767.
    0,
    807,
    1614,
    2420,
    3224,
    4027,
    4827,
    5624,
    6417,
    7207,
    7993,
    8773,
    9548,
    10318,
    11081,
    11837,
    12586,
    13328,
    14061,
    14786,
    15502,
    16209,
    16906,
    17592,
    18268,
    18932,
    19586,
    20227,
    20856,
    21472,
    22076,
    22666,
    23242,
    23804,
    24351,
    24884,
    25402,
    25904,
    26391,
    26861,
    27315,
    27753,
    28174,
    28578,
    28964,
    29333,
    29684,
    30017,
    30331,
    30628,
    30905,
    31164,
    31404,
    31625,
    31827,
    32009,
    32172,
    32316,
    32440,
    32544,
    32628,
    32693,
    32738,
    32762,
    32767,
    32752,
    32718,
    32663,
    32588,
    32494,
    32380,
    32247,
    32093,
    31921,
    31728,
    31517,
    31287,
    31037,
    30769,
    30482,
    30176,
    29852,
    29510,
    29151,
    28773,
    28378,
    27966,
    27536,
    27090,
    26628,
    26149,
    25655,
    25145,
    24620,
    24079,
    23525,
    22955,
    22372,
    21776,
    21166,
    20543,
    19908,
    19261,
    18602,
    17931,
    17250,
    16558,
    15857,
    15145,
    14425,
    13696,
    12958,
    12213,
    11460,
    10700,
    9934,
    9161,
    8383,
    7600,
    6813,
    6021,
    5226,
    4427,
    3626,
    2822,
    2017,
    1211,
    404,
    -404,
    -1211,
    -2017,
    -2822,
    -3626,
    -4427,
    -5226,
    -6021,
    -6813,
    -7600,
    -8383,
    -9161,
    -9934,
    -10700,
    -11460,
    -12213,
    -12958,
    -13696,
    -14425,
    -15145,
    -15857,
    -16558,
    -17250,
    -17931,
    -18602,
    -19261,
    -19908,
    -20543,
    -21166,
    -21776,
    -22372,
    -22955,
    -23525,
    -24079,
    -24620,
    -25145,
    -25655,
    -26149,
    -26628,
    -27090,
    -27536,
    -27966,
    -28378,
    -28773,
    -29151,
    -29510,
    -29852,
    -30176,
    -30482,
    -30769,
    -31037,
    -31287,
    -31517,
    -31728,
    -31921,
    -32093,
    -32247,
    -32380,
    -32494,
    -32588,
    -32663,
    -32718,
    -32752,
    -32767,
    -32762,
    -32738,
    -32693,
    -32628,
    -32544,
    -32440,
    -32316,
    -32172,
    -32009,
    -31827,
    -31625,
    -31404,
    -31164,
    -30905,
    -30628,
    -30331,
    -30017,
    -29684,
    -29333,
    -28964,
    -28578,
    -28174,
    -27753,
    -27315,
    -26861,
    -26391,
    -25904,
    -25402,
    -24884,
    -24351,
    -23804,
    -23242,
    -22666,
    -22076,
    -21472,
    -20856,
    -20227,
    -19586,
    -18932,
    -18268,
    -17592,
    -16906,
    -16209,
    -15502,
    -14786,
    -14061,
    -13328,
    -12586,
    -11837,
    -11081,
    -10318,
    -9548,
    -8773,
    -7993,
    -7207,
    -6417,
    -5624,
    -4827,
    -4027,
    -3224,
    -2420,
    -1614,
    -807,
    0,
};


int main(int argc, char const *argv[])
{

    for(float i=0; i < TWO_PI_F; i+=0.1){

        int16_t I_u = (uint16_t) (2048 * sin(i)); 
        int16_t I_v = (uint16_t) (2048 * sin(i - 2.09439f)); 
        int16_t I_w = (uint16_t) (2048 * sin(i - 4.18879f)); 

        uint8_t angle_lut = (uint8_t) (i / TWO_PI_F * 255 * 2);

        //each term below has 16 fractional bits and is signed
        int16_t Q16_sin_t = sin_lut[angle_lut];
        int16_t Q16_cos_t = sin_lut[(64 - angle_lut) & (256 - 1)]; //64 out of 256 is the equilvalent of 90º/360º. Modulo 256.

        int16_t Q16_SQRT3_2_sin_t = (Q16_SQRT3_2*Q16_sin_t) >> 16;
        int16_t Q16_SQRT3_2_cos_t = (Q16_SQRT3_2*Q16_cos_t) >> 16;
        int16_t Q16_1_2_sin_t = (Q16_1_2*Q16_sin_t) >> 16;
        int16_t Q16_1_2_cos_t = (Q16_1_2*Q16_cos_t) >> 16;

        int16_t I_d = ( Q16_cos_t*I_u + ( Q16_SQRT3_2_sin_t - Q16_1_2_cos_t)*I_v + (-Q16_SQRT3_2_sin_t - Q16_1_2_cos_t)*I_w) >> 15;
        I_d = (I_d * Q16_2_3) >> 16;
        int16_t I_q = ( Q16_sin_t*I_u + (-Q16_SQRT3_2_cos_t - Q16_1_2_sin_t)*I_v + ( Q16_SQRT3_2_cos_t - Q16_1_2_sin_t)*I_w) >> 15;
        I_q = (I_q * -Q16_2_3) >> 16; 



        // printf("%d %d %d %d \n", Q16_sin_t, Q16_cos_t, I_d, I_q);


        //Reference floating point
        float angle_f = (angle_lut * TWO_PI_F) / 255.0f;
        float sf = sin(angle_f);
        float cf = cos(angle_f);

        float I_d_f = 0.6666667f*(cf*I_u + (SQRT3_2*sf-.5f*cf)*I_v + (-SQRT3_2*sf-.5f*cf)*I_w);   ///Faster DQ0 Transform
        float I_q_f = 0.6666667f*(-sf*I_u - (-SQRT3_2*cf-.5f*sf)*I_v - (SQRT3_2*cf-.5f*sf)*I_w);

        // float I_d_f = 0.6666667f*(cf*I_u);   ///Faster DQ0 Transform
        // float I_q_f = 0.6666667f*(-sf*I_u);

        // printf("%f %f %f %f \n", 32768*sf, 32768*cf, I_d_f, I_q_f);

        //compare:
        printf("%d %d %d %d %f %f %f %f \n", Q16_sin_t, Q16_cos_t, I_d, I_q, 32768*sf, 32768*cf, I_d_f, I_q_f);
    
    }

    return 0;
}
